...
pub fn render(
    ...
    if let Some(area) = list_area {
        ...
        if !app.services.iam.view_mode.is_main_tab() {
            let title = match app.services.iam.view_mode {
                IamViewMode::UserAttachedPolicies => format!(
                ...
                IamViewMode::RoleAttachedPolicies => format!(
                ...
                IamViewMode::PolicyDocument => format!(
                    "Policy Document: {}",
                    app.services
                        .iam
                        .selected_entity_name
            ...
            let tabs: Vec<&str> = crate::app::IamViewMode::iterator()
                .map(|m| m.label())
                .collect();
            crate::ui::components::tabs::render_tabs(
                ...
                chunks[0],
                &tabs,
                app.services.iam.view_mode.index(),
            );
        }
        ...
        match app.services.iam.view_mode {
            ...
            IamViewMode::UserAttachedPolicies | IamViewMode::RoleAttachedPolicies => {
                render_attached_policies_list(frame, chunks[1], app)
            }
    ...
    if let Some(area) = detail_area {
        match app.services.iam.view_mode {
            ...
            IamViewMode::UserAttachedPolicies | IamViewMode::RoleAttachedPolicies => {
                render_policy_details_from_list(frame, area, app)
            }
...
fn render_user_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select an IAM user to view details (use j/k to navigate)",
        )]
    };
...
fn build_user_detail_lines(user: &IamUser) -> Vec<Line<'static>> {
    ...
    let arn_line = Line::from(vec![Span::styled(
        user.arn.clone().unwrap_or_else(|| "-".into()),
    ...
    DetailBuilder::new()
        ...
        .field_owned(
            "Created",
            user.create_date.clone().unwrap_or_else(|| "-".into()),
        )
        .field_owned(
            "Password Last Used",
...
fn render_role_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select an IAM role to view details (use j/k to navigate)",
        )]
    };
...
fn build_role_detail_lines(role: &IamRole) -> Vec<Line<'static>> {
    ...
    // ARN on separate line with styled value
    let arn_line = Line::from(vec![Span::styled(
        role.arn.clone().unwrap_or_else(|| "-".into()),
        Style::default().fg(THEME.selection_fg),
    )]);
    ...
    DetailBuilder::new()
        ...
        .field_owned(
            "Created",
            role.create_date.clone().unwrap_or_else(|| "-".into()),
        )
...
fn render_policy_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select an IAM policy to view details (use j/k to navigate)",
        )]
    };
...
fn build_policy_detail_lines(policy: &IamPolicy) -> Vec<Line<'static>> {
    ...
    // ARN on separate line with styled value
    let arn_line = Line::from(vec![Span::styled(
        policy.arn.clone().unwrap_or_else(|| "-".into()),
        Style::default().fg(THEME.selection_fg),
    )]);
    ...
    DetailBuilder::new()
        ...
        .field_owned(
            "Created",
            policy.create_date.clone().unwrap_or_else(|| "-".into()),
        )
        .field_owned(
            "Updated",
            policy.update_date.clone().unwrap_or_else(|| "-".into()),
        )
        .blank()
        .field("ARN", "")
        .raw_line(arn_line)
...
fn render_attached_policies_list(frame: &mut Frame, area: Rect, app: &mut App) {
    let rows = app.services.iam.current_policies.iter().map(|policy| {
        let cells = vec![
            Cell::from(policy.policy_name.clone()),
