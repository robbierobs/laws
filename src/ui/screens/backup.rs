...
pub fn render(
    ...
    if let Some(area) = list_area {
        ...
        let tabs: Vec<&str> = crate::app::BackupViewMode::iterator()
            .map(|m| m.label())
            .collect();
        crate::ui::components::tabs::render_tabs(
            ...
            chunks[0],
            &tabs,
            app.services.backup.view_mode.index(),
        );
...
fn render_vault_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select a backup vault to view details (use j/k to navigate)",
        )]
    };
...
fn render_plan_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select a backup plan to view details (use j/k to navigate)",
        )]
    };
...
fn render_job_list(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if has_filters {
        ...
        let state = state_labels
            .get(current_filters.state_index)
            .unwrap_or(&"All");
...
fn render_job_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select a backup job to view details (use j/k to navigate)",
        )]
    };
    ...
    let rows = app
        ...
        .map(|rp| {
            ...
            let cells = vec![
                ...
                Cell::from(rp.status.clone().unwrap_or_default())
                    .style(Style::default().fg(status_color)),
                Cell::from(size),
                Cell::from(created),
                Cell::from(
                    rp.recovery_point_arn
                        .split('/')
                        .next_back()
                        .unwrap_or("?")
                        .to_string(),
...
fn render_recovery_point_details(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let content: Vec<Line> = if let Some(idx) = selected {
        ...
        vec![Line::from(
            "Select a recovery point to view details (use j/k to navigate)",
        )]
    };
...
fn build_recovery_point_detail_lines(rp: &RecoveryPoint) -> Vec<Line<'_>> {
    ...
    vec![
        Line::from(vec![
            ...
            Span::styled(
                rp.recovery_point_arn.split('/').next_back().unwrap_or("?"),
                Style::default()
                    .fg(THEME.selection_fg)
                    .add_modifier(Modifier::BOLD),
            ),
        ...
        Line::from(vec![
            ...
            Span::styled(
                rp.status.clone().unwrap_or_default(),
