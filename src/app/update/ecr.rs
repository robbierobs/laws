...
impl App {
    ...
    fn load_ecr_images(
        ...
        let handle = tokio::spawn(async move {
            ...
            match ecr_service.describe_images_paginated(
                &repo_name,
                Some(max_results),
                next_token.as_deref(),
                tag_filter.as_deref(),
    ...
    fn handle_pull_image(
        ...
        self.action_log.push(format!("Pulling image: {}", image_uri));
        self.loading = true;
        ...
        let handle = tokio::spawn(async move {
            ...
            let (username, password, proxy_endpoint) = match ecr_service.get_authorization_token().await {
                Ok(auth) => auth,
                Err(e) => {
            ...
            let login_result = tokio::process::Command::new("docker")
                .args(["login", "--username", &username, "--password-stdin", &proxy_endpoint])
